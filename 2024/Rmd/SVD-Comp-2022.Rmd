...
```{r echo=FALSE}
# record when this starts
write(Sys.time(),file="../Rmd/started.txt")
...
The data come from the Human Mortality Database (HMD) and are available online at [www.mortality.org - **click here**](www.mortality.org). To get the latest HMD, go to the HMD web site, sign up for an account, and download the 'All HMD statistics zip file'. The code below is used to process the contents of that file to extract the life tables.

```{r access-data-functions, eval = TRUE, echo = TRUE}
...
The following chunk unzips the version of HMD from 2018 that was used in the *Demography* paper. The data are unzipped into the *data/HMD/hmd_statistics* directory.

```{r unzip-hmd, eval = TRUE, echo = TRUE, results = 'hide'}
...
ggplot(data = am.cm, aes(x=CM, y=AM, group=Type, colour=Type)) + 
	geom_point(size=0.2) +
	labs(y = expression(bold('Adult Mortality ')
	                    [bold(45)]*bolditalic('q')[bold(15)]*bold(' (logit scale)'))
	     , x = expression(bold('Child Mortality ')
	                      [bold(5)]*bolditalic('q')[bold(0)]*bold(' (logit scale)'))) +
...
write(Sys.time(),file="../Rmd/stopped.txt")
