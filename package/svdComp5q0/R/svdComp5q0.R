#' Child/Child-Adult Mortality-indexed Model Life Tables.
#'
...
#' @examples
#' svdComp5q0("female",0.05,TRUE,FALSE,TRUE)
#' svdComp5q0("male",0.03,TRUE,FALSE,TRUE,0.26)
#' svdComp5q0("male",0.03,TRUE,TRUE,TRUE,0.26)
#' svdComp5q0("male",c(0.03,0.01),am=0.3)
#' @importFrom stats predict
#' @export
svdComp5q0 <- function(sex,cm,smooth=TRUE,logit=FALSE,out5=TRUE,am=NULL) {
  ...
  if (smooth) {
    cs <- mods[[sex]]$comps.sm
  } else {
    cs <- mods[[sex]]$comps
  }

  # calcualte predicted qx values
  v <- cbind(v1,v2,v3,v4)
...
# #' @param smooth Boolean: use either smooth or raw SVD-derived components. Default is TRUE.
# #' @param logit Boolean: output either logit scale or natural scale 1qx values. Default is FALSE.
# #' @param am Optional decimal: input value(s) for 45q15; either single value or vector of values.
# #' @return Data frame: predicted 1qx values for ages 0:109. Age 110 assumed to be 1.0 and not returned. Columns labeled with input child mortality values.
# #' @examples
# #' svdComp5q0("female",0.05,TRUE,FALSE)
# #' svdComp5q0("male",0.03,TRUE,FALSE,0.26)
# #' @importFrom stats predict
# #' @export
# svdComp5q0 <- function(sex,cm,smooth=TRUE,logit=FALSE,am) {
#
#   # sex: "female" or "male"
#   # cm is a vector of 5q0 values
#   # smooth: boolean, use smooth components or not
...
#
#
#   if(tolower(sex) == "female" | tolower(sex) == "male") {
#
#     # transform inputs
#     cml <- logit(cm)
#     cmls <- cml^2
...
#
#   } else {
#     print("Sex must be 'female' or 'male'.")
#   }
#
# }
